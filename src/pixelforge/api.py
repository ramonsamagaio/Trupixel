...
app = FastAPI(title="TruPixel", version="0.1.0", description="Headless pixel-art reconstruction with privacy-preserving community learning.")
store = JobStore()

@app.get("/health")
def health():
    return {"ok": True, "service": "trupixel"}
...
def feedback(job_id: str, outcome: str = Form(...)):
    if outcome not in {"accepted", "rejected", "reverted"}: raise HTTPException(400, "outcome must be accepted, rejected, or reverted")
